...
const Productpage = () => {
	...
	return (
		<>
			<article className="container mx-auto flex flex-col items-center justify-center gap-8 px-4 py-16 md:flex-row">
				<div
				...
				<div className="flex flex-col items-center justify-center gap-4 sm:items-start">
					...
					<div className="flex flex-col items-center justify-center gap-4 sm:h-[50px] sm:flex-row">
						<div className="divide-primary-1 border-primary-1 flex h-12 w-[200px] justify-center gap-0 divide-x-2 overflow-clip rounded-lg border-2 text-center text-3xl">
							<button
								className="h-full w-1/2 font-medium text-skin-secondary-2 transition-all duration-150 hover:bg-skin-secondary-2 hover:text-skin-text"
								onClick={() => setItems(clamp(items - 1, 1, 10))}
							>
								-
							</button>
							<p className="flex h-full w-1/2 items-center justify-center">{items}</p>
							<button
								className="h-full w-1/2 font-medium text-skin-secondary-2 transition-all duration-150 hover:bg-skin-secondary-2 hover:text-skin-text"
								onClick={() => setItems(clamp(items + 1, 1, 10))}
							>
						...
						<Button asChild className="w-fit p-0 transition-all">
							<CartButton
					...
					<div className="divide-primary-1 border-primary-1 w-full divide-y-2 rounded-lg border-2">
						<div className="flex justify-start gap-4 p-8">
							<DeliveryIcon />
